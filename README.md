...
```json
{
  ...
  "factors": [
    ...
    },
    {
      ...
      "topPercent": 12,
      "changePercent": 18.3,
      "description": "시청 지속 시간이 매우 길어 콘텐츠 몰입도가 높습니다."
    },
    {
      "name": "추천 확장성",
      "score": 22,
      "topPercent": 78,
      "changePercent": -55.0,
      "description": "추천 확장성이 낮습니다. 시청 유지율과 참여도를 높여보세요."
    }
  ],
...
| `factors` | ✅ | CTR (클릭률) / 시청 지속 시간 / 추천 확장성 카드 3개. Analytics 미연동 시 빈 배열 |
...
| `factors[].description` | ✅ | 점수대별 설명 문장 (각 지표별 4구간) |
...
| `audienceRetention.mainDropOffSegment.startSeconds` | ✅ | 주요 이탈 구간 시작 (초) |
| `audienceRetention.mainDropOffSegment.endSeconds` | ✅ | 주요 이탈 구간 끝 (초) |
...
### 2026-05-12
- **영상 상세 분석 API 응답 정리**
  - `uploadDate` 형식 변경: `yyyy-MM-dd` → `yyyy.MM.dd` (점 구분)
  - `factors[].name`: `"CTR"` → `"CTR (클릭률)"` 으로 변경
  - `factors[].rawValue` 필드 제거 (API 응답에서만 제거, AI 프롬프트용 내부 유지)
  - `audienceRetention.avgRetentionPercent` 필드 제거
  - `audienceRetention.mainDropOffSegment`에서 `startLabel`, `endLabel`, `sectionLabel` 필드 제거
  - `channelRank` 필드 제거 (rank, total, betterThanPercent)
  - 각 factor별 description 메서드 추가 (`getCtrDescription`, `getWatchDurationDescription`, `getRecommendDescription`)
  - `likeCount`, `commentCount`, `channelId`, `channelName` 필드 제거
